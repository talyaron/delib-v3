...
module.exports = {
    ...
    view: (vnode) => {
        ...
        return (
            <div class={vnode.state.owned?'card groupCard questionOwned':'card groupCard'} onclick={() => { m.route.set(vnode.attrs.route + vnode.attrs.question.id) }}>
...
function checkIfGroupOwnedByUser(vnode){
    if(vnode.attrs.hasOwnProperty('creatorId') && vnode.attrs.question.creatorId == store.user.uid){
